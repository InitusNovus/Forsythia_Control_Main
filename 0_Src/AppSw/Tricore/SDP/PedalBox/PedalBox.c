...
//TODO: Elimination of BPPS part? BSE is governed by RVC.
/******************************************************************************/
/*-----------------------------------Macros-----------------------------------*/
/******************************************************************************/
#if PPSMODE == PWM
...
#elif PPSMODE == ADC
// FIXME: All big letter is only for MACRO. It will be renamed as "appsX".
AdcSensor APPS0;
AdcSensor APPS1;
//FIXME: There is no BPPS.\
	There is BSEs, which are an pressure sensor and they do not match eachother.
AdcSensor BPPS0;
AdcSensor BPPS1;
#endif
...
void SDP_PedalBox_init(void)
{
	...
	// FIXME: Temporary value for overvoltage limits. Must be set correctly.
	config_adc.overvoltageLimitConfig.limHi = 4.5;
	config_adc.overvoltageLimitConfig.limLo = 0.5;
...
{
	data_out->pedalPercent = data_out->config.reversed ? (float32)100.0 - SDP_PedalBox_getPedalLinear(data_out, data_in)
...
IFX_STATIC void SDP_PedalBox_updatePPS_AN(SDP_PedalBox_sensor_t *data_out, AdcSensor *data_in)
{
	...
	data_out->pedalPercent = data_out->config.reversed ? (float32)100.0 - data_in->value : data_in->value;
}
#endif
...
IFX_STATIC void SDP_PedalBox_checkErrorState_fromThree(
    SDP_PedalBox_sensor_t *data1, SDP_PedalBox_sensor_t *data2, SDP_PedalBox_sensor_t *data3)
{
